...
async def animelogo(_, message):
        ...
        LOGO_API_URL0 = f"https://api.sdbots.tk/anime-logo?name={text}"
        url = requests.get(LOGO_API_URL0).url
        await x.edit("`🔄 Done Generated... Now Sending You`")
        await message.reply_photo(url,caption="**🖼 Logo Generated By @Nandhabots**",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Upload As File 📁", callback_data=f"flogo {logo_id}")]]))
        await x.delete()
